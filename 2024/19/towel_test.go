...
func Test_isPossible(t *testing.T) {
	t.Run("example", func(t *testing.T) {
		...
		assert.True(t, isPossible(memo, towels, "brwrr"))
		assert.True(t, isPossible(memo, towels, "bggr"))
		assert.True(t, isPossible(memo, towels, "gbbr"))
		assert.True(t, isPossible(memo, towels, "rrbgbr"))
		assert.False(t, isPossible(memo, towels, "ubwu"))
		assert.True(t, isPossible(memo, towels, "bwurrg"))
		assert.True(t, isPossible(memo, towels, "brgr"))
		assert.False(t, isPossible(memo, towels, "bbrgwb"))
	})
	...
	t.Run("scenario 1", func(t *testing.T) {
		...
		memo := map[string]bool{}
		isPossible(memo, towels, "wwggwbwruubbwurrgrggbuuwwwgwbrwubggurbrwugguruggggbrwggbbrg")
	})
}
